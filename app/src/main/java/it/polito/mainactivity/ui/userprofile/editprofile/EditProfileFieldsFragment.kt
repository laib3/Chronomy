...
class EditProfileFieldsFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        vm.user.observe(viewLifecycleOwner) {
            ...
            //    vm.resetUpdated()
            //}
            binding.textInputEditTextName.setText(it.name)
            binding.textInputEditTextSurname.setText(it.surname)
            binding.textInputEditTextNickname.setText(it.nickname)
            binding.textInputEditTextBio.setText(it.bio)
            binding.textInputEditTextPhone.setText(it.phone)
            binding.textInputEditTextEmail.setText(it.email)
            binding.textInputEditTextLocation.setText(it.location)
        ...
        binding.textInputEditTextName.setOnFocusChangeListener { _, focused ->
            if (!focused) {
                if (vm.user.value?.name.toString() != binding.textInputEditTextName.text.toString()) {
                    ...
                    vm.user.value?.let { vm.updateTimeslotField(it.uid, "name", binding.textInputEditTextName.text.toString())}
                    (parentFragment as EditProfileFragment).notifyMessageEditedProfile()
                }
        ...
        binding.textInputEditTextSurname.setOnFocusChangeListener { _, focused ->
            if (!focused) {
                if (vm.user.value?.surname.toString() != binding.textInputEditTextSurname.text.toString()) {
                    ...
                    vm.user.value?.let { vm.updateTimeslotField(it.uid, "surname", binding.textInputEditTextSurname.text.toString())}
                    (parentFragment as EditProfileFragment).notifyMessageEditedProfile()
                }
        ...
        binding.textInputEditTextNickname.setOnFocusChangeListener { _, focused ->
            if (!focused) {
                if (vm.user.value?.nickname.toString() != binding.textInputEditTextNickname.text.toString()) {
                    ...
                    vm.user.value?.let { vm.updateTimeslotField(it.uid, "nickname", binding.textInputEditTextNickname.text.toString())}
                    (parentFragment as EditProfileFragment).notifyMessageEditedProfile()
                }
        ...
        binding.textInputEditTextBio.setOnFocusChangeListener { _, focused ->
            if (!focused) {
                if (vm.user.value?.bio.toString() != binding.textInputEditTextBio.text.toString()) {
                    ...
                    vm.user.value?.let { vm.updateTimeslotField(it.uid, "bio", binding.textInputEditTextBio.text.toString())}
                    (parentFragment as EditProfileFragment).notifyMessageEditedProfile()
                }
        ...
        binding.textInputEditTextPhone.setOnFocusChangeListener { _, focused ->
            if (!focused) {
                if (vm.user.value?.phone.toString() != binding.textInputEditTextPhone.text.toString()) {
                    ...
                    vm.user.value?.let { vm.updateTimeslotField(it.uid, "phone", binding.textInputEditTextPhone.text.toString())}
                    (parentFragment as EditProfileFragment).notifyMessageEditedProfile()
                }
        ...
        binding.textInputEditTextEmail.setOnFocusChangeListener { _, focused ->
            if (!focused) {
                if (vm.user.value?.email.toString() != binding.textInputEditTextEmail.text.toString()) {
                    ...
                    vm.user.value?.let { vm.updateTimeslotField(it.uid, "email", binding.textInputEditTextEmail.text.toString())}
                    (parentFragment as EditProfileFragment).notifyMessageEditedProfile()
                }
        ...
        binding.textInputEditTextLocation.setOnFocusChangeListener { _, focused ->
            if (!focused) {
                if (vm.user.value?.location.toString() != binding.textInputEditTextLocation.text.toString()) {
                    ...
                    vm.user.value?.let { vm.updateTimeslotField(it.uid, "location", binding.textInputEditTextLocation.text.toString())}
                    (parentFragment as EditProfileFragment).notifyMessageEditedProfile()
                }
